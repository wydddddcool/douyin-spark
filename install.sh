...
# 读取实际端口（launcher 启动后写入；launchd 启动用 plist 里的 5001）
PORT=5001
if [ -f "$PROJ_DIR/auth/web_port.txt" ]; then
  PORT=$(cat "$PROJ_DIR/auth/web_port.txt" 2>/dev/null | tr -d '[:space:]')
  [ -z "$PORT" ] && PORT=5001
fi

for i in 1 2 3 4 5; do
  if curl -sf "http://127.0.0.1:$PORT/api/status" > /dev/null 2>&1; then
    echo "  ✅ Web 服务已启动 (端口 $PORT)"
    break
  fi
  # 兜底：探测常见端口
