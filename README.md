...
基于 Playwright + 系统 Chrome（不触发人机验证），多 provider AI 消息生成 + 本地消息池兜底，反风控设计（随机打字、好友间随机间隔、定时 jitter），跨平台（macOS / Windows / Linux / Docker）。

---

## ✨ 特性

- 🤖 **Playwright + 系统 Chrome** — 不被抖音人机验证拦截（Selenium 路线已失效）
